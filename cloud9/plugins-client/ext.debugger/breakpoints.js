...
module.exports = {
    hook: function() {
        ...
        /* Prevent the IDE from loading breakpoints from the .session file 
        ...
        dbgBreakpoints.addEventListener("afterrender", function() {
            lstBreakpoints.$ext.addEventListener("click", function(e) {
                ...
                if (className.indexOf("btnclose") != -1) {
                    ...
                    /* Remove breakpoint from the config file of th the currently
                     * selected workspace */
                    var data = { command: "firmware",
                                 "operation": "removeBreakpoint",
                                 "breakpointToDelete": selected.getAttribute("path")+":"+selected.getAttribute("line"),
    ...
    removeBreakpoint: function(path, row) {
        ...
        var data = { command: "firmware",
                     "operation": "removeBreakpoint",
                     "breakpointToDelete": path+":"+row,
                      requireshandling: true
                   };
        ide.send(data);
        
    },
    /* Remove a breakpoint from the grid (dock) only */
    removeBreakpointFromGrid: function(path, row) {
        this.$updating = true;
        var bp = mdlDbgBreakpoints.queryNode("breakpoint[@path=" + util.escapeXpathString(path) +
            " and @line='" + row + "']");
        bp && apf.xmldb.removeNode(bp);
    ...
    addBreakpointToModel: function(session, row) {
        ...
        /* Add breakpoint to the config file of th the currently
         * selected workspace */
        var data = { command: "firmware",
                     "operation": "saveBreakpoint",
                     "breakpoint": path+":"+row,
                      requireshandling: true
                   };
        ide.send(data);
