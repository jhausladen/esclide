    ...
    openSelection : function(){
        ...
        sel.forEach(function(node){
            if (!node || node.tagName != "file")
                return;
            /* Get the path of the control-flow graph file */
            var controlFlowGraphPath = trFiles.selected.getAttribute("path");

            /* Check the file suffixes for control-flow graph files */
            var suffix = controlFlowGraphPath.match(/\..*/,'');
                 
            /* If the file is a control-flow graph open it in the html previewer
               Otherwise open the file in the editor */           
            if(suffix != null && suffix.indexOf(".cfg.html") > -1)preview.preview(location.protocol + "//" + location.host + controlFlowGraphPath);
            else editors.gotoDocument({node: node});
        });
    },
